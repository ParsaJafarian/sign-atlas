 ...
 */
import { Enrollment, validateCompletedLessons, validateEnrollmentsUniqueness } from '@/db/enrollment';
 ...
 */
userSchema.methods.getEnrollmentByCourseId = function (this: IUser, courseId: string) {
    const enrollment = this.enrollments.find(enrollment => enrollment.course.toString() === courseId);
    if (!enrollment) throw new Error('Course not found in enrollments of current user');
    return enrollment;
}
...
userSchema.methods.isEnrolledIn = function (this: IUser, courseId: string) {
    return this.enrollments.some(enrollment => enrollment.course.toString() === courseId);
}
 ...
 * @param courseId CourseId of the course in string format
 */
userSchema.methods.addEnrollment = async function (this: IUser, courseId: string) {
